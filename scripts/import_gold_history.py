...
# Config
DOWNLOADED_CSV = r"C:\Users\User-PC\Downloads\XAU_USD Historical Data.csv"
...
def import_history():
    ...
    with open(DOWNLOADED_CSV, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            raw_price = row[1].replace(",", "")  # "4,929.94"
            ...
            try:
                ...
                # Entry for 999 (MYR/g)
                if (timestamp, "world_gold", "999") not in existing_records:
                    new_entries.append([timestamp, "world_gold", "999", str(myr_g), str(myr_g), "0.0"])
                
                # Entry for USD/oz
                if (timestamp, "world_gold", "USD/oz") not in existing_records:
                    new_entries.append([timestamp, "world_gold", "USD/oz", str(usd_oz), str(usd_oz), "0.0"])
    ...
    with open(HISTORY_CSV, "a", encoding="utf-8", newline='') as f:
        ...
        for entry in new_entries:
            ...
            print(f"Imported: {entry[0]} | {entry[2]} | {entry[3]}")

    print(f"\nSuccessfully imported {len(new_entries)} records into {HISTORY_CSV}.")
